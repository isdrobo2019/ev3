...
# 全てのブロックを移動する際の最適なルート探索を行う
def get_all_route(start_idx, direction, block_setting_list_org, block_circle_list_org, bonus_num):
    result = []
    loop_cnt = 0
    # 本体の向き(参照渡し用に配列として定義)
    ...
    block_circle_list_in = copy.copy(block_circle_list_org)
    # ブロック単位でループ
    while 1:
        # 現状の最適ルート検索
